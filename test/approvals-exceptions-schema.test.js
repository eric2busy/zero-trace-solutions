...
test('approval decisions are append-only and browser-denied', () => {
...
test('approval validity check fails closed on scope, status, expiry, and receipt', () => {
  ...
  assert.match(sql, /revoke all on function public\.command_approval_allows_action/i);
});
